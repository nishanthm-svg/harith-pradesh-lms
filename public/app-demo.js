// ============================================================================
// Read-only content DEMO build — no login, no accounts, no admin portal, no
// server. Progress lives only in memory for this tab and resets on reload.
// Reuses ./data.js, ./i18n.js and ./styles.css unchanged from the real app.
// ============================================================================
...
// ============================================================================
// In-memory-only progress (no server, no localStorage) — same shape/behaviour
// as the real app's progress store, just never persisted anywhere.
// ============================================================================
let progress = {}; // { moduleId: { lessonId: {completed,bestScore,attempts} } }

function getLessonState(moduleId, lessonId) {
  return (progress[moduleId] && progress[moduleId][lessonId]) || { completed: false, bestScore: 0, attempts: 0 };
}

function recordQuizAttempt(moduleId, lessonId, scorePercent, passed) {
  ...
  progress[moduleId][lessonId] = next;
  return next;
}

function isLessonUnlocked(mod, lessonId) {
...
// Landing page
// ============================================================================
function demoText(en, hi) {
  return lang === "hi" ? hi : en;
}

function renderLandingPage() {
  const features = [
    { icon: "📚", title: demoText("12 Modules", "12 मॉड्यूल"), text: demoText("From dairy basics to milk pricing, MPP routes, and cost management.", "डेयरी की मूल बातों से लेकर दूध की कीमत, एमपीपी मार्ग और लागत प्रबंधन तक।") },
    { icon: "🌐", title: demoText("English & Hindi", "अंग्रेज़ी और हिन्दी"), text: demoText("Learn in English or Hindi — whichever you're comfortable with.", "अंग्रेज़ी या हिन्दी में सीखें — जो भी आपके लिए आसान हो।") },
    { icon: "🔍", title: demoText("Preview Only", "केवल प्रीव्यू"), text: demoText("This is a read-only content demo — no login and progress isn't saved.", "यह एक रीड-ओनली कंटेंट डेमो है — कोई लॉगिन नहीं है और प्रगति सहेजी नहीं जाती।") },
  ]
    .map(
      (f) => `
  ...
  return `
    <div class="landing-page">
      <div class="landing-hero">
        ...
        <h1>${escapeHtml(demoText("Learn to Serve Farmers Better — At Your Own Pace", "किसानों की बेहतर सेवा करना सीखें — अपनी गति से"))}</h1>
        <p>${escapeHtml(demoText("A preview of the Harith Pradesh employee training course. No login needed — explore Module 1 freely.", "हरित प्रदेश कर्मचारी प्रशिक्षण पाठ्यक्रम का एक प्रीव्यू। लॉगिन की ज़रूरत नहीं — मॉड्यूल 1 खुलकर देखें।"))}</p>
        <button type="button" class="btn btn-primary landing-cta" id="landing-start-btn">${escapeHtml(demoText("Start Demo →", "डेमो शुरू करें →"))}</button>
      </div>
...
function wireLanguagePicker(isSwitcher, returnHash) {
  document.querySelectorAll("[data-lang]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      setLang(lang);
      navigate(isSwitcher && returnHash ? returnHash : "#/dashboard");
    });
  });
...
function renderTopbar(context) {
  ...
  return `
    <div class="topbar">
      ...
      <span class="admin-nav-btn" style="cursor:default;">${escapeHtml(demoText("Demo", "डेमो"))}</span>
...
function renderDashboard() {
  const cards = MODULES.map((mod) => {
    ...
    const progressInfo = getModuleProgress(mod);
    const badge = progressInfo.isComplete
    ...
    return `
      <div class="module-card available" data-nav="#/module/${mod.id}">
        ...
        <div class="module-info">
          <h3>${escapeHtml(t(mod.title))}</h3>
          <div class="meta">${u("lessonsCompleteMeta", { completed: progressInfo.completed, total: progressInfo.total })}</div>
          <div class="mini-bar"><div style="width:${progressInfo.percent}%"></div></div>
        </div>
        ${badge}
      </div>
    `;
  ...
  return `
    ...
    <div class="page">
      ...
      <div class="progress-note">${escapeHtml(demoText("Demo preview — progress resets when you reload the page.", "डेमो प्रीव्यू — पेज रीलोड करने पर प्रगति रीसेट हो जाती है।"))}</div>
    </div>
  `;
}
    ...
    <div class="page">
      <div class="module-hero">
        ...
        <div class="progress-row">
          <div class="bar-track"><div style="width:${progressInfo.percent}%"></div></div>
          <div class="progress-label">${u("moduleProgressComplete", { completed: progressInfo.completed, total: progressInfo.total })}</div>
        </div>
      </div>
    ...
    flowEl.innerHTML = `
      <div class="quiz-section">
        <div class="quiz-result pass">
          ...
          <div class="score-circle"><div class="pct">${scorePercent}%</div><div class="lbl"></div></div>
...
// ============================================================================
// Router — no auth, no admin. Only welcome/language/dashboard/module/lesson/complete.
// ============================================================================
function parseHash() {
